...
if (s.x < map[0].length - 1 && map[s.y][s.x + 1].links.includes(D.W))
  allowed.push(D.E);
if (s.y > 0 && map[s.y - 1][s.x].links.includes(D.S)) allowed.push(D.N);
if (s.y < map.length - 1 && map[s.y + 1][s.x].links.includes(D.N))
  allowed.push(D.S);
...
console.log(
  "p1",
  Math.max(...map.flat().map((p) => (p.dist === undefined ? 0 : p.dist)))
);
...
for (let x = 0; x < map[0].length; x++) {
  if (map[0][x].dist === undefined)
    stack.push({ p: { x: x * 3, y: 0 }, dist: 0 });
  if (map[map.length - 1][x].dist === undefined)
    stack.push({ p: { x: x * 3, y: (map.length - 1) * 3 }, dist: 0 });
}

for (let y = 1; y < map.length - 1; y++) {
  if (map[y][0].dist === undefined)
    stack.push({ p: { x: 0, y: y * 3 }, dist: 0 });
  if (map[y][map[0].length - 1].dist === undefined)
    stack.push({ p: { x: (map[0].length - 1) * 3, y: y * 3 }, dist: 0 });
}

while (stack.length) {
  let n = stack.shift();
  if (
    maze[n.p.y] !== undefined &&
    maze[n.p.y][n.p.x] !== undefined &&
    maze[n.p.y][n.p.x] !== 1 &&
    mazeDist[n.p.y][n.p.x] === undefined &&
    (mazeDist[n.p.y][n.p.x] === undefined || mazeDist[n.p.y][n.p.x] > n.dist)
  ) {
...
//     if (v == 0) return true;
//     ctx.fillStyle =
//       v == 1 ? "#000" : mazeDist[y][x] === undefined ? "#f00" : "#00f";
